...
<div id="midd">
	<div class="inln">
		<h1 style="font-weight:bold;">Cargo Flight Availability Today</h1>
		<h4>filter by</h4>
		<form action="flight_c.php" method="post">
		<a class="inl">
		<p>flight</p>

		<select name="plane_id">
				<?php
					if($_POST["plane_id"]!='plane_id')
						echo "<option value='".$_POST['plane_id']."'>".$_POST['plane_id']."</option>";

				?>
				<option value="plane_id">none</option>
			
				  <optgroup label="Cargo">
				    <?php
					$sql = "select plane_id from plane natural join typer natural join cargo order by plane_id";
						$result = $conn->query($sql);
						if ($result->num_rows > 0) {
						    // output data of each row
						    while($row = $result->fetch_assoc()) {
						        echo '<option value="'.$row["plane_id"].'">'.$row["plane_id"].'</option>';
						    }
						}
				?>
	...
	<div class="inln">
		...
		<?php
			...
			$sql = "select source,airline_name as an,load_height as h,color as c,load_lenght as l,load_breadth as b,plane_id,model_name,destination,departure,arrival,load_capacity as cap from services natural join plane_type natural join journey natural join service_availability natural join owns natural join airlines natural join plane natural join typer natural join cargo where day_avail=dayname(current_timestamp) and model_id>=459 and plane_id=".$_POST['plane_id']." and destination=".$_POST['destination']." order by ".$_POST['sortby'].";";
				echo "<h4>filtered for Flight :".$_POST['plane_id']." and Destination: ".$_POST['destination']." sorted on: ".$_POST['sortby']." </h4>";
			      ...
			      while($row = $result->fetch_assoc()) {
					 ...
					 echo '</td>
					  ...
					  echo '</tr>';
			      		...
			      		echo "<p style='background-color:#cccccc; margin:5px;'>Service not offered by selected flight to selected destination Today</p>";
