...
<div id="midd">
	<div class="inln">
		<h1 style="font-weight:bold;">Flight Availability Today</h1>
		<h4>filter by</h4>
		...
		<select name="plane_id">
				...
				<option value="plane_id">none</option>
				  ...
				  <optgroup label="passenger">
				    <?php
					$sql = "select plane_id from plane natural join typer natural join passenger_plane order by plane_id";
	...
	var x;
	function hov(){
		 x=document.getElementById("trr").innerHTML;
		 
		document.getElementById("trr").style="margin:15px";

	}
	function outt(){
		document.getElementById("trr").innerHTML=x;
	}
	</script>
	<div class="inln" >
		...
		<?php
			...
			$sql = "select source,airline_name as an,color as c,busi_class_capacity as bc,eco_class_capacity as ec,plane_id,destination,departure,arrival from services natural join passenger_plane natural join owns natural join airlines natural join plane natural join journey natural join service_availability natural join typer where day_avail=dayname(current_timestamp) and model_id<459 and plane_id=".$_POST['plane_id']." and destination=".$_POST['destination']." order by ".$_POST['sortby'].";";
			  ...
			  if ($result->num_rows > 0) {
				...
				echo $time; 
			      ...
			      while($row = $result->fetch_assoc()) {
			          ...
			          echo  '<tr  id="trr" onmouseover="hov()" onmouseout="outt()">
					  <td>';
					 	if($row['source']==''){
					 		echo "Beijing";
					 	}
